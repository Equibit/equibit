...
def main():
    ...
    configfile = os.path.abspath(os.path.dirname(__file__)) + "{}..{}config.ini".format(dir_separator, dir_separator)
    ...
    tmpdir = "%sbitcoin_test_runner_%s" % (args.tmpdirprefix + dir_separator, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    if len(test_list) > 1 and jobs > 1:
        # Populate cache
        python_exe = ["python"] if sys.platform == "win32" else []
        try:
            subprocess.check_output(python_exe + [tests_dir + 'create_cache.py'] + flags + ["--tmpdir={}{}cache".format(tmpdir, dir_separator)])
...
class TestHandler:
    ...
    def get_next(self):
        ...
        while self.num_running < self.num_jobs and self.test_list:
            ...
            test_argv = t.split()
            testdir = "{}{}{}_{}".format(self.tmpdir, dir_separator, re.sub(".py$", "", test_argv[0]), portseed)
            ...
            self.jobs.append((t,
                              time.time(),
                              subprocess.Popen(python_exe + [self.tests_dir + test_argv[0]] + test_argv[1:] + self.flags + portseed_arg + tmpdir_arg,
                                               universal_newlines=True,
                                               stdout=log_stdout,
                                               stderr=log_stderr),
                                               #executable="C:\\Users\\Alex\\AppData\\Local\\Programs\\Python\\Python37\\python.exe"),
                              testdir,
                              log_stdout,
                              log_stderr))
        if not self.jobs:
