(set _ROOT=%CD%\..\..)
(set QT_ROOT=C:\source\libraries\qt)
(set QT_BUILD_ROOT=C:\source\libraries\qt-build)
(set QT_PLUGIN_PATH=%QT_BUILD_ROOT%\qtbase\plugins)
(set VCPKG=%_ROOT%\..\vcpkg)
(set PROTOBUF_TOOL=%VCPKG%\packages\protobuf_x64-windows-static\tools\protobuf)
(set SRC_PATH=%_ROOT%)
(set OUT_PATH=%CD%\build)

for %%p in (protoc.exe) do set "protocpath=%%~$PATH:p"
if not defined protocpath (
   set "PATH=%PATH%;%PROTOBUF_TOOL%"
)

for %%p in (moc.exe) do set "mocpath=%%~$PATH:p"
if not defined mocpath (
   set "PATH=%PATH%;%QT_BUILD_ROOT%\qtbase\bin"
)

for %%p in (lrelease.exe) do set "lreleasepath=%%~$PATH:p"
if not defined lreleasepath (
   set "PATH=%PATH%;%QT_BUILD_ROOT%\qttools\bin"
)

if not exist "%OUT_PATH%" mkdir "%OUT_PATH%"
if not exist "%OUT_PATH%\qt" mkdir "%OUT_PATH%\qt"
if not exist "%OUT_PATH%\qt\forms" mkdir "%OUT_PATH%\qt\forms"

forfiles /p %SRC_PATH%\src\qt\locale\ /m *.ts /c "cmd /c lrelease  @file -qm @fname.qm"

forfiles /p %SRC_PATH%\src\qt\forms\ /m *.ui /c "cmd /c uic  @file -o %OUT_PATH%\qt\forms\ui_@fname.h"
...
moc %MOC_DEF%  %SRC_PATH%\src\qt\bantablemodel.h -o %OUT_PATH%\moc_bantablemodel.cpp
...
moc %MOC_DEF%  %SRC_PATH%\src\qt\modaloverlay.h -o %OUT_PATH%\moc_modaloverlay.cpp
...
moc %MOC_DEF%  %SRC_PATH%\src\qt\bitcoin.cpp -o %OUT_PATH%\qt\bitcoin.moc
moc %MOC_DEF%  %SRC_PATH%\src\qt\bitcoinamountfield.cpp -o %OUT_PATH%\qt\bitcoinamountfield.moc
moc %MOC_DEF%  %SRC_PATH%\src\qt\intro.cpp -o %OUT_PATH%\qt\intro.moc
moc %MOC_DEF%  %SRC_PATH%\src\qt\overviewpage.cpp -o %OUT_PATH%\qt\overviewpage.moc
moc %MOC_DEF%  %SRC_PATH%\src\qt\rpcconsole.cpp -o %OUT_PATH%\qt\rpcconsole.moc

protoc -I=%SRC_PATH%\src\qt\ --cpp_out=%OUT_PATH%\qt\ %SRC_PATH%\src\qt\paymentrequest.proto
